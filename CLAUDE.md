...
- 固有 ID は使用中（弓を引く・食べる・盾構え等）の手のアイテムには初回スタンプを見送る（`getActiveItemHand()` で使用中スロットを判定）。書き戻し（`setItem`）で使用アクションを中断しないため。そのアイテムの通知は使用をやめた次のスキャンまで保留される（盾を構え続ける等、使い続けている間は通知されない）。
