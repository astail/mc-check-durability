...
1. `DuraAlert-1.1.0.jar` を `plugins/` に置いてサーバーを再起動します。
...
```bash
./deploy.sh        # Mac ネイティブ（JDK 25 + Maven）。生成物: target/DuraAlert-1.1.0.jar
# または
mvn -B clean package
```
...
[ビルド](#ビルド) の手順で `target/DuraAlert-1.1.0.jar` を生成します。
...
cp target/DuraAlert-1.1.0.jar /path/to/data/plugins/
docker restart <コンテナ名>

# 名前付きボリューム等の場合（コンテナへ直接コピー）
docker cp target/DuraAlert-1.1.0.jar <コンテナ名>:/data/plugins/
docker restart <コンテナ名>
```
...
services:
  mc:
    ...
    environment:
      ...
      PLUGINS: |
        https://github.com/astail/mc-check-durability/releases/download/v1.1.0/DuraAlert-1.1.0.jar
    volumes:
      - ./data:/data
    restart: unless-stopped
```

`PLUGINS` は改行区切りで複数指定できます。バージョンを更新したら、URL の `v1.1.0` とファイル名を新しいリリースに合わせて変更してください（例: `.../download/v1.1.0/DuraAlert-1.1.0.jar`）。
